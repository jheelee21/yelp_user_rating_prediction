...
og_model <- lm(average_stars ~ review_count + fans + average_useful +
                 elite + average_business_stars, data = data)
...
```{r}
bc_average_business_stars <- data$average_business_stars ** 7/3
bc_review_count <- data$review_count ** -1/3
```

```{r}
model2 <- lm(average_stars ~ bc_review_count + fans + average_useful +
               elite + bc_average_business_stars, data = data)
...
plot(resid(model3) ~ bc_average_business_stars, main = "All Transformed")
plot(resid(model1) ~ data$average_business_stars, main = "Transformed Y")
plot(resid(og_model) ~ data$average_business_stars, main = "Untransformed")
```